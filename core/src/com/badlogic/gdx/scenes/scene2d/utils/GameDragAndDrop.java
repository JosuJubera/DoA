        ...
        DragListener listener = new DragListener() {
            ...
            public void drag (InputEvent event, float x, float y, int pointer) {
                ...
                for (int i = 0, n = targets.size; i < n; i++) {
                    ...
                    Vector2 screencoords = new Vector2(stageX, stageY);
                    event.getStage().stageToScreenCoordinates(screencoords);

//                        if (!target.actor.isAscendantOf(hit)) continue;
//                        newTarget = target;
                    //target.actor.stageToLocalCoordinates(tmpVector.set(stageX, stageY));
                    tmpVector.set(target.actor.screenToLocalCoordinates(screencoords));

                    if(target.actor.hit(tmpVector.x, tmpVector.y, true) != null) {
