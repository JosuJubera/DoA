...
import com.aro.defenseofatroth.Actors.Canon;
import com.aro.defenseofatroth.Actors.Enemy;
...
//	private Vector2 camposicion;
	...
	*/
	...
	public Level2(MainClass game, boolean musica) {
		super(game);
		this.game = game;
		create();
...
//		camposicion = new Vector2(0f, 0f);
		...
		Texture torreTex = MainClass.getManager().get("torre.png", Texture.class);
		torre = new Canon(torreTex);
		torre.debug();

		stage.addActor(torre);

		Texture enemyTex = MainClass.getManager().get("barraRoja.png", Texture.class);
	...
	private void comprobarColision() {
		if (enemy.isAlive() && (torre.getX() < enemy.getX() + enemy.getWidth())) {
		//	torre.fire();
